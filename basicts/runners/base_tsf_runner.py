...
class BaseTimeSeriesForecastingRunner(BaseRunner):
    ...
    def train_iters(self, epoch: int, iter_index: int, data: Union[torch.Tensor, Tuple]) -> torch.Tensor:
        ...
        # Calculate loss in normalized space to prevent overfitting
        if self.cl_param:
            cl_length = self.curriculum_learning(epoch=epoch)
            loss_forward_return = [forward_return[0][:, :cl_length, :, :], forward_return[1][:, :cl_length, :, :]]
        else:
            loss_forward_return = forward_return[:2]
        loss = self.metric_forward(self.loss, loss_forward_return)
        
        # re-scale data for metrics only
        ...
        if self.cl_param:
            cl_length = self.curriculum_learning(epoch=epoch)
            metric_forward_return = [prediction_rescaled[:, :cl_length, :, :], real_value_rescaled[:, :cl_length, :, :]]
        else:
            metric_forward_return = [prediction_rescaled, real_value_rescaled]

        for metric_name, metric_func in self.metrics.items():
            metric_item = self.metric_forward(metric_func, metric_forward_return)
